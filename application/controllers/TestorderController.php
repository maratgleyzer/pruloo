...
class TestorderController extends Zend_Controller_Action
{
...
//		var_dump($order);exit;	
			...
			if (($storedOrder['impulse'] > 0) && ($order['affi_id'] != 12474) && eregi("SALE",$response)) {
				$impulse = $this->SendLeadToImpulse($storedOrder, 2);
				$impulse = $this->SendSaleToImpulse($storedOrder);
			}

			if (($storedOrder['upsl_id'] > 0) && ($order['affi_id'] != 12474) && eregi("SALE",$response))
				$response = "UPSELL";
...
//		}
	...
	private function ParseOrderData($post) {
    	...
    	foreach ($post as $key => $value) {
    		...
    		$order[substr(ltrim(rtrim($key)),3)] = $value;
    	}
